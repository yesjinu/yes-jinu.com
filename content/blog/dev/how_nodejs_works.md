---
title: Node.js의 실행구조 분석
date: 2021-06-29 23:06:55 +0900
category: dev
thumbnail: { nodejs.svg }
draft: false
---

너무도 많은 사람들이 사용하지만, 대부분의 사람들이 내부적으로 어떻게 작동하는지 알지 못한채로 사용하는 Node.js. 오늘은 node.js 인터페이스에서 딱 한 단계 더 깊이 들어가 node.js를 분석하고자합니다. 이하 글에서는 편의를 위해 node.js를 '노드'라고 부를게요 :) 

![img](../img/nodejs.svg)

## Node.js에 대한 간략한 설명
노드는 2009년 JSConf EU에서 라이언 달이라는 사람에 의해 처음 공개되었습니다. 당시에 가장 널리 쓰이던 웹 서버는 Apache HTTP 서버였는데, 이 프로그램은 **concurrent connection을 핸들링하는 데에 한계**가 있었습니다. 내부적으로 synchronous하게 동작하도록 짜여있어 하나의 큰 long-running function를 실행하다가 전체 프로세스가 멈춰버리는 경우가 많았다고 하네요. 
...
이 두 가지 사실을 조합해 정리하자면, 노드는 당시의 웹 서버가 가진 문제점을 해결하기 위해 고안된 asynchronous하게 동작하는 JS 런타임 환경입니다. 지금 당장은 이해가 가지 않아도 좋습니다 :) 이 포스팅을 다 읽고난 이후에는 위 문장을 이해하게 되실거에요.
...
먼저, 세부사항에 대해 알아보기 전에 노드 내부의 큼직한 구조도를 살펴보면 이해에 많은 도움이 될 것입니다. 노드는 여러 가지의 큰 모듈들의 집합으로 구성되어 있는데요, 이중에서 주요한 역할을 하는 두 가지의 큰 기둥이 V8과 libuv입니다. 

![image](../img/nodejs_module_diagram.jpg)

- V8은 구글이 개발한 오픈소스 JS 엔진으로 JS를 native C++ 코드로 컴파일하는 역할을 맡습니다. V8은 자체적으로 70% C++와 30%의 JS로 구성되어 있습니다. 
- libuv는 비동기 I/O operation을 구현하는 엔진입니다. 100% C로 짜여져 있습니다.
결국 우리가 살펴볼 두 가지 문제에 대한 해답을 쥐고 있는 두 가지 모듈이라고 할 수 있겠습니다. 그럼 하나씩 살펴볼게요. 우리가 앞서 물었던 문제는 결국 'JS 코드와 V8, libuv 컴포넌트가 서로 어떻게 상호작용하는지'에 그 해답이 있었습니다.

> 이 포스팅에서는 V8과 libuv가 어떻게 구현되어 있는지는 살펴보지 않고, 전체적인 실행 흐름에 집중해 다루려합니다.



## 1. 노드는 JS를 어떻게 실행하는가?
> TLDR; V8엔진이 JS 코드를 C++로 컴파일. 노드가 제공하는 모듈은 process.binding() 과정을 통해 C++ 코드를 연결되어 있음.


[Github repository of Node.js](https://github.com/nodejs/node)

노드의 소스코드에는 크게 2개의 메인 디렉토리가 있습니다.
- `lib` : JS 코드를 작성하는 데에 필요한 definition이 저장. 이외에도 node가 제공하는 라이브러리들이 있음(fs, crypto, path, http, etc.)
- `src` : C++로 구현. V8 엔진의 C++로 구현된 JS object(Array, Boolean)를 import해서 사용. lib 엔진을 이용해 OS에 access.

파일 시스템(fs)를 예시로 들어 동작을 설명하려 합니다. `fs.readFile`은 JS도 V8에도 속하지 않는 구현체로, 노드가 제공하는 함수입니다. 이 함수가 어떻게 실행되는지 살펴봄으로써 JS로 작성한 코드가 어떻게 V8과 연결되는지 살펴볼 수 있을 것입니다.
...
> TLDR; 이벤트 루프를 통해 I/O operation을 워커 스레드에 위임. 결과값을 나중에 받아보는 식

### 자바스크립트 런타임
먼저 자바스크립트 런타임에 대해 알아야 합니다. 자바스크립트는 싱글 스레드로 작동합니다. 이 말의 뜻은 하나의 프로세스에서 실행되는 자바스크립트 엔진(V8)은 하나의 `콜 스택`만을 제공한다는 것과 같습니다.
...
### 이벤트 루프
이벤트 루프는, (대부분 멀티 스레드로 동작하는) 시스템 커널에 업무를 분담함으로써 노드가 blocking 되어 있지 않고도 I/O operation을 수행할 수 있게 해줍니다. 커널에 업무를 위임하고, 완료되었는지 주기적으로 체크하는 식으로 작동하는 것이지요.  

![image](../img/nodejs_event_loop.jpg)
...
## 마무리
이렇게 노드가 내부적으로 동작하는 방식을 한 단계 깊게 들어가 알아보았습니다. 물론 이것 역시 꽤나 많은 부분이 추상화되어 있는 high-level에서의 분석이지만, 노드의 동작을 이해할 수 있는 첫 걸음이 되었기를 바랍니다. 추후 포스팅에서 더 자세한 내용을 다뤄보겠습니다 :)
